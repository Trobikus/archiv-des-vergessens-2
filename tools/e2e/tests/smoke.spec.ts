...
test("client smoke: intro → character select", async ({ page }) => {
  ...
  // Boot may finish before the full intro: skip appears. Or intro auto-exits
  // to the character select — the game boots straight into local play.
  await Promise.race([
